...
<%@page import="com.quickelp.programa.persistencia.dao.ReparacionDAO"%>
<%@page import="com.quickelp.programa.persistencia.vo.ReparacionVO"%>
...
<html lang="es-CO">
    ...
    <!-- ****************************************************************************************************************** -->    
    <head>
        <title>Lista de Reparaciones realizados por <%out.print(usu.getNombre());%> <%out.print(usu.getApellido());%>.</title>
        <!-- Required meta tags -->
        <meta charset="utf-8">
                ...
                <div class="navbar-menu-wrapper d-flex align-items-stretch">
                    <div class="search-field d-none d-md-block">
                        <form class="d-flex align-items-center h-100" action="#">
                            <h3>Lista de reparaciones realizados por ${nombre}</h3></form>
                    </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <table class="table table-responsive">
                                        <thead>
                                            <tr>
                                                <th>Cons.</th>
                                                <th>Estado</th>
                                                <th>Fecha Recepcion</th><!-- Opcional -->
                                                <th>Solucion Reparacion</th>
                                                <th>Fecha Entrega</th>
                                        ...
                                        <%
                                            ReparacionVO rep = null;
                                            ReparacionDAO repDAO = new ReparacionDAO();
                                            List<ReparacionVO> listaRepTec = repDAO.consultarReparaciones(Integer.parseInt(request.getParameter("id")));
                                            Iterator<ReparacionVO> iteraRep = listaRepTec.iterator();
                                            while (iteraRep.hasNext()) {
                                                rep = iteraRep.next();
                                        %>
                                        <tbody>
                                            <tr>
                                                <td><%=rep.getIdReparacion()%></td>
                                                <td><%=rep.getIdServicio().getIdEstado().getNombreEstado()%></td>
                                                <td><%=rep.getFechaRecepcion()%></td>
                                                <td><%=rep.getSolucion()%></td>
                                                <td><%=rep.getFechaEntrega()%></td>

                                                <td><%=rep.getIdServicio().getIdUsuario().getNombre()%></td>
                                                <td><%=rep.getIdServicio().getIdUsuario().getApellido()%></td>
                                                <td><%=rep.getIdServicio().getIdUsuario().getTelefono()%></td>
                                                <td><%=rep.getIdServicio().getIdUsuario().getDireccion()%></td>
                                                <td><%=rep.getIdServicio().getDescripcion()%></td>

                                                <!--Equipo-->
                                                <td><%=rep.getIdServicio().getIdEquipo().getTipoEquipo()%></td>
                                                <td><%=rep.getIdServicio().getIdEquipo().getIdmarca().getNombreMarca()%></td>
                                                <td><%=rep.getIdServicio().getIdEquipo().getModelo()%></td>
                                                <td><%=rep.getIdServicio().getIdEquipo().getNumSerial()%></td>
                                                <td><%=rep.getIdServicio().getIdTipoSer().getIdTipoSer().getNomTipoServicio()%></td>

                                                <td>
                                                    <a class="badge badge-info" href="Diagnostico?accion=registrar_informe&idg=<%=Integer.toString(rep.getIdReparacion())%>&id=${idUsu}"><i class="mdi mdi-account-card-details"></i></a>
                                                    <a class="badge badge-success" href="Reparacion?accion=registrar_informe&idh=<%=Integer.toString(rep.getIdReparacion())%>&id=${idUsu}"><i class="mdi mdi-wrench"></i></a>
                                    ...
                                    <div class="template-demo">
                                        ...
                                        <span class="nav-link">
                                            ...
                                            <a class="btn btn-fw btn-outline-info mt-4" href="Reparacion?accion=registrar_tecnico&id=${idUsu}"><i class="mdi mdi-wrench"></i>+ Nueva Reparacion</a>              
                                        </span>
